...
int main()
{
    ...
    {
        string curr = prefix.substr(0, i);

        TrieNode* found = search(root, curr);

        if(found -> ch != curr[curr.length() - 1])
        cout << "NO SUCH PREFIX: ";
        else
        {
            cout << "Current Substring Prefix: " << curr << endl;
            findPrefix(found, curr);
        }
    }
